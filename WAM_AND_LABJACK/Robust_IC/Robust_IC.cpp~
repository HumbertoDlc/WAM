...
int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam) 
{
	...
	// Desired impedance:
	double M_des[4]={50,5,2,2};					//{5,5,2,2};
	double K_des[4]={20,120,1200,150};				//{1600,50,1000,90};
	double B_des[4]={20,30,80,50};					//{100,30,80,50};
	double Kf_des=1;
	...
	for (int i=0;i<txt_long;++i){
		...
		//
		J[0][1]=(11*cos(q1)*sin(q2))/20 - (9*sin(q1)*sin(q3))/200 + (9*cos(q4)*(sin(q1)*sin(q3) - cos(q1)*cos(q2)*cos(q3)))/200 - 
		(sin(q4)*(sin(q1)*sin(q3) - cos(q1)*cos(q2)*cos(q3)))/2 + (9*cos(q1)*cos(q2)*cos(q3))/200 + (cos(q1)*cos(q4)*sin(q2))/2 + (9*cos(q1)*sin(q2)*sin(q4))/200;
		J[1][1]=sin(q1)*((11*cos(q2))/20 + (cos(q2)*cos(q4))/2 - (9*cos(q3)*sin(q2))/200 + (9*cos(q2)*sin(q4))/200 + (9*cos(q3)*cos(q4)*sin(q2))/200 - 
		(cos(q3)*sin(q2)*sin(q4))/2);
		...
		J_FT[1]=0;J_FT[2]=0;J_FT[3]=0;		
		J_FT_TEST[j] =(J[j][0]*Fx+J[j][1]*Fy+J[j][2]*Fz+J[j][3]*Tx+J[j][4]*Ty+J[j][5]*Tz);		
		...
		//cout<<"T_2_a: "<<T_2_a<<"\nT_2_b: "<<T_2_b<<"\nT_2_c: "<<T_2_c<<"\nT_2_d: "<<T_2_d<<"\nT_2_e: "<<T_2_e<<"\nT_2_f: "<<T_2_f<<"\nT_2_g: "<<T_2_g<<"\nT_2_h: "<<T_2_h<<"\ni: "<<i<<"\n\n\n\n\n\n";
		//cout<<"T_4_a: "<<T_4_a<<"\nT_4_b: "<<T_4_b<<"\nT_4_c: "<<T_4_c<<"\nT_4_d: "<<T_4_d<<"\nT_4_e: "<<T_4_e<<"\nT_4_f: "<<T_4_f<<"\nT_4_g: "<<T_4_g<<"\nT_4_h: "<<T_4_h<<"\ni: "<<i<<"\n\n\n\n\n\n";	
		cout<<"J_FT: "<<J_FT_TEST[0]<<", "<<J_FT_TEST[1]<<", "<<J_FT_TEST[2]<<", "<<J_FT_TEST[3]<<".\n";
		//cout<<"Torques: "<<T_1<<", "<<T_2<<", "<<T_3<<", "<<T_4<<".\n";
			Torques[0]=T_1;
			Torques[1]=T_2;
			Torques[2]=T_3;
			Torques[3]=T_4;
